...
internal class ArithmeticScannerTest {
    ...
    """.trimIndent()
        ...
        val tokens = arithmeticScanner.scan(test, 0, 0)
        ...
        assertEquals(Token(Tokens.AND, "and "), tokens[3])
        ...
        assertEquals(Token(Tokens.AND, "and "), tokens[11])
    ...
    fun testRemainderExpression() {
        val test = "24%32%46==t"
        val arithmeticScanner = ArithmeticScanner()
        val result = arithmeticScanner.scan(test, 0, 0)
    ...
    fun testPlusExpression() {
        ...
        val result = arithmeticScanner.scan(test, 0, 0)
        assertEquals(Token(Tokens.IDENTIFIER, "24 "), result[0])
        assertEquals(Token(Tokens.PLUS, "+"), result[1])
        assertEquals(Token(Tokens.IDENTIFIER, "  65 "), result[2])
        assertEquals(Token(Tokens.EQUAL, "=="), result[3])
    ...
    fun testScanWithDoubleAnd() {
        ...
        assertEquals(Token(Tokens.EQUAL, "=="), result[1])
        assertEquals(Token(Tokens.IDENTIFIER, " 2 "), result[2])
        assertEquals(Token(Tokens.AND_DOUBLE, "&& "), result[3])
